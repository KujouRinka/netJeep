...
void TCPIn::closeMe(CloseType type) {
  ...
  error_code err;
  switch (type) {
    case CloseType::read:_in_sock->shutdown(ip::tcp::socket::shutdown_receive, err);
      break;
    case CloseType::write:_in_sock->shutdown(ip::tcp::socket::shutdown_send, err);
      break;
    case CloseType::both:_in_sock->cancel(err);
      _in_sock->close(err);
      break;
  }
}
...
void TCPOut::closeMe(CloseType type) {
  ...
  error_code err;
  switch (type) {
    case CloseType::read:_out_sock->shutdown(ip::tcp::socket::shutdown_receive, err);
      break;
    case CloseType::write:_out_sock->shutdown(ip::tcp::socket::shutdown_send, err);
      break;
    case CloseType::both:_out_sock->cancel(err);
      _out_sock->close(err);
      break;
  }
}
